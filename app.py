...
with tab6:
    ...
    deptos_str = ", ".join(deptos) if len(deptos) <= 5 else f"{len(deptos)} departamentos"
    sup_media  = int(total_sup / n_prod) if n_prod > 0 else 0
    pct_univ   = f"{total_stock_filt/total_ovinos_univ*100:.1f}%" if col_stock=='total ovinos' and total_ovinos_univ > 0 else "—"
    ...
    if año_ant:
        ...
        if stock_ant > 0:
            ...
            arrow = "▲" if var >= 0 else "▼"
            clr   = "#2ea043" if var >= 0 else "#da3633"
            delta_html = f'<div style="font-size:.78rem;color:{clr};margin-top:.2rem">{arrow} {abs(var):.1f}% vs {año_ant}</div>'
    ...
    for i, row in top5.iterrows():
        ...
        rows_top5 += (
            ...
            f'<td style="padding:.45rem .7rem;border-bottom:1px solid #f0f0f0">{i+1}</td>'
            f'<td style="padding:.45rem .7rem;border-bottom:1px solid #f0f0f0"><strong>{row["establecimiento"]}</strong></td>'
            f'<td style="padding:.45rem .7rem;border-bottom:1px solid #f0f0f0">{row["departamento"]}</td>'
            f'<td style="padding:.45rem .7rem;border-bottom:1px solid #f0f0f0">{row["cuit cuil"]}</td>'
            f'<td style="padding:.45rem .7rem;border-bottom:1px solid #f0f0f0;text-align:right"><strong>{int(row[col_stock]):,}</strong></td>'
            f'<td style="padding:.45rem .7rem;border-bottom:1px solid #f0f0f0;text-align:right">{int(row["superficie"]):,} ha</td>'
            f'</tr>'
        )

    # ── Filas tabla departamentos ───────────────────────────────────
    rows_dist = ""
    for i, row in dist_ej.iterrows():
        pct = row['stock'] / total_stock_filt * 100 if total_stock_filt > 0 else 0
        bg  = '#fafafa' if i % 2 == 0 else '#ffffff'
        rows_dist += (
            f'<tr style="background:{bg}">'
            f'<td style="padding:.45rem .7rem;border-bottom:1px solid #f0f0f0">{row["departamento"]}</td>'
            f'<td style="padding:.45rem .7rem;border-bottom:1px solid #f0f0f0;text-align:right">{int(row["prod"]):,}</td>'
            f'<td style="padding:.45rem .7rem;border-bottom:1px solid #f0f0f0;text-align:right">{int(row["stock"]):,}</td>'
            f'<td style="padding:.45rem .7rem;border-bottom:1px solid #f0f0f0;text-align:right">{pct:.1f}%</td>'
            f'</tr>'
        )

    # ── HTML completo ───────────────────────────────────────────────
    TH = 'style="background:#1a1a2e;color:#fff;padding:.5rem .7rem;text-align:left;font-weight:600"'
    TH_R = 'style="background:#1a1a2e;color:#fff;padding:.5rem .7rem;text-align:right;font-weight:600"'
...
<style>
  * {{ box-sizing:border-box; margin:0; padding:0; }}
  ...
  .wrap {{ background:#fff; color:#1a1a2e; padding:2.5rem 3rem; border-radius:12px;
           max-width:860px; margin:0 auto; box-shadow:0 4px 24px rgba(0,0,0,.1); }}
  ...
  .hdr .sub {{ font-size:.88rem; color:#555; margin-top:.3rem; }}
  .meta {{ font-size:.78rem; color:#666; text-align:right; line-height:1.8; }}
  .sec {{ margin:1.4rem 0; }}
  .sec h2 {{ font-size:.78rem; font-weight:700; text-transform:uppercase; letter-spacing:.08em;
             color:#888; border-bottom:1px solid #e5e7eb; padding-bottom:.3rem; margin-bottom:.9rem; }}
  .grid {{ display:grid; grid-template-columns:repeat(4,1fr); gap:.7rem; }}
  .kpi {{ background:#f8fafc; border:1px solid #e5e7eb; border-radius:8px; padding:.8rem 1rem; }}
  .kpi .lbl {{ font-size:.68rem; color:#888; text-transform:uppercase; letter-spacing:.05em; }}
  .kpi .val {{ font-size:1.45rem; font-weight:700; color:#1a1a2e; margin-top:.15rem; }}
  table {{ width:100%; border-collapse:collapse; font-size:.82rem; }}
  .footer {{ margin-top:2rem; padding-top:1rem; border-top:1px solid #e5e7eb;
             font-size:.7rem; color:#aaa; display:flex; justify-content:space-between; }}
  @media print {{
    body {{ background:white; padding:0; }}
    .wrap {{ box-shadow:none; padding:1.5rem; }}
  }}
</style>
</head>
<body>
<div class="wrap">
  ...
  <div class="hdr">
    <div>
      ...
      <div class="sub">Informe ejecutivo · Ciclo <strong>{año_sel}</strong> · {tipo_stock} ≥ <strong>{int(min_animales):,}</strong> cabezas</div>
    </div>
    <div class="meta">
  ...
  <div class="sec">
    ...
    <div class="grid">
      <div class="kpi"><div class="lbl">Establecimientos</div><div class="val">{n_prod:,}</div></div>
      <div class="kpi"><div class="lbl">CUITs únicos</div><div class="val">{cuits_unicos:,}</div></div>
      <div class="kpi"><div class="lbl">Stock {tipo_stock}</div><div class="val">{total_stock_filt:,}</div>{delta_html}</div>
      <div class="kpi"><div class="lbl">Superficie total</div><div class="val">{total_sup:,} ha</div></div>
    </div>
    <div class="grid" style="margin-top:.7rem">
      <div class="kpi"><div class="lbl">Ovinos universo (año)</div><div class="val">{total_ovinos_univ:,}</div></div>
      <div class="kpi"><div class="lbl">Bovinos universo (año)</div><div class="val">{total_bovinos_univ:,}</div></div>
      <div class="kpi"><div class="lbl">Stock filtro / universo</div><div class="val">{pct_univ}</div></div>
      <div class="kpi"><div class="lbl">Sup. media (ha/estab.)</div><div class="val">{sup_media:,}</div></div>
    </div>
  </div>
  ...
  <div class="sec">
    ...
    <table>
      <thead><tr>
        <th {TH}>#</th>
        <th {TH}>Establecimiento</th>
        <th {TH}>Departamento</th>
        <th {TH}>CUIT</th>
        <th {TH_R}>{tipo_stock}</th>
        <th {TH_R}>Superficie</th>
      </tr></thead>
  ...
  <div class="sec">
    ...
    <table>
      <thead><tr>
        <th {TH}>Departamento</th>
        <th {TH_R}>Establecimientos</th>
        <th {TH_R}>{tipo_stock}</th>
        <th {TH_R}>% del total</th>
      </tr></thead>
  ...
  <div class="footer">
    ...
    <span>Ciclo {año_sel} · {tipo_stock} ≥ {int(min_animales):,} · {deptos_str}</span>
  </div>
    ...
    altura = max(900, 420 + len(dist_ej) * 32 + 5 * 36)
    components.html(html, height=altura, scrolling=True)

    st.info("💡 **Para exportar a PDF:** **Ctrl+P** (Win) o **Cmd+P** (Mac) → *Guardar como PDF* en el diálogo del navegador.")
